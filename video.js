...
//     objectDetector.detect(c1, (err, results) => {
//         console.log(results); // Will output bounding boxes of detected objects
//         personCount = 0; // Reset count for each frame
//         for (let index = 0; index < results.length; index++) {
//             const element = results[index];
//             if (element.label === "person") {
//                 personCount++; // Increment count if a person is detected
//             }
//             ctx1.font = "15px Arial";
//             ctx1.fillStyle = "red";
//             ctx1.fillText(element.label + " - " + (element.confidence * 100).toFixed(2) + "%", element.x + 10, element.y + 15);
...
//             console.log(element.label);
//         }
//         // Update the content of the span element with the person count
//         document.getElementById("personCount").innerText = "Number of persons detected: " + personCount;
//     });
// }
var personCount = 0;

document.getElementById("saveButton").addEventListener("click", savePhoto);
...
function savePhoto() {
    ...
    // Save the image data with the current time as the file name
    var datetime = new Date().toLocaleString().replace(/[\/\s,:]/g, "-");
...
function ai() {
    ...
    objectDetector.detect(c1, (err, results) => {
        ...
        // Update the content of the span element with the person count
        document.getElementById("personCount").innerText = "Хүмүүсын тоо: " + personCount;
    });
}
